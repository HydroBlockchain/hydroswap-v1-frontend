...
export const getSouschefContract = (id: number, signer?: ethers.Signer | ethers.providers.Provider) => {
  ...
  const abi = config ? config.poolCategory === PoolCategory.BINANCE ? sousChefBnb : sousChef : {}
  return getContract(abi, config ? getAddress(config.contractAddress) : '', signer) // Briws: type fix
}
export const getSouschefV2Contract = (id: number, signer?: ethers.Signer | ethers.providers.Provider) => {
  const config = poolsConfig.find((pool) => pool.sousId === id)
  return getContract(sousChefV2, config ? getAddress(config.contractAddress) : '', signer) // Briws: type fix
}
...
export const getErc721CollectionContract = (signer?: ethers.Signer | ethers.providers.Provider, address?: string) => {
  return getContract(erc721CollectionAbi, address || '', signer) // Briws: type fix
}
