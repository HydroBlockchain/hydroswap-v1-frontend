...
export const nonArchivedFarms = farmsConfig.filter(({ pid }) => !isArchivedPid(pid || 0))
...
>(
  ...
  async (pids) => {
    const farmsToFetch = farmsConfig.filter((farmConfig) => pids.includes(farmConfig.pid ? farmConfig.pid : 0))
